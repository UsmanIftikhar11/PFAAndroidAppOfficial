...
public class CustomViewCreate extends SearchBizData implements BizLocCallback {
    ...
    private boolean isFat_rendering_license_type;
    ...
    public void createViews(final FormSectionInfo formSectionInfo, final LinearLayout parentView, final HashMap<String, HashMap<String, Boolean>> sectionRequired, final PFAViewsCallbacks pfaViewsCallbacks2, boolean setLocationListener, final ScrollView mainScrollView , Context activity) {
        ...
        if (formSectionInfo.getAdd_new() != null) {
            ...
            setAddDynamicItemJSONObj(formSectionInfo.getAdd_new(), null, new CreateViewCallback() {
                ...
                public void createAddView(FormSectionInfo formSectionInfo, LinearLayout parentView) {
                    createViews(formSectionInfo, parentView, sectionRequired, pfaViewsCallbacks2, false, null , mContext);
                }
            });
        }
...
//                get_code_button, text, textarea, numeric, cnic ,phone, email, dropdown , radiogroup, checkbox, label, date, imageView,  button,  autoSearch, location_fields
            switch (fieldInfo.getField_type()) {
                ...
                case "multiMap":
                    Log.d("viewCreated", "multigooglemap");
                    createMultiGoogleMap(parentView, inflater, fieldInfo);
                    break;
...
/*
                ...
                case "ratingBar":
                    ...
                    createRatingBar(formSectionInfo, fieldInfo, parentView, imageLayout, sectionRequired, pfaViewsCallbacks2, inflater);
                    break;
...
//plus2
        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                PFAEditText revised_license_typeDD = parentView.findViewWithTag("revised_license_type");
                if (fat_rendering_license_typeDD != null && fat_rendering_license_typeDD.getVisibility() == VISIBLE) {
                    String DDVal = fat_rendering_license_typeDD.getText().toString();
                    if (DDVal == null || DDVal.isEmpty()) {
                        sharedPrefUtils.showMsgDialog("PLease select license type", null);
                    } else {
...
//                    String confirmMsg1 = confirmRevisedCatMsg.substring(confirmRevisedCatMsg.lastIndexOf("Bu") , confirmRevisedCatMsg.lastIndexOf(")"));
                ...
                } else if (revised_license_typeDD != null && revised_license_typeDD.getVisibility() == VISIBLE) {
                    String DDVal = revised_license_typeDD.getText().toString();
                    if (DDVal == null || DDVal.isEmpty()) {
                        sharedPrefUtils.showMsgDialog("PLease populate license type", null);
                    } else {
                        if (confirmRevisedCatMsg != null && !confirmRevisedCatMsg.isEmpty()) {
                            String confirmMsg = String.format(confirmRevisedCatMsg.replace("\\n", System.lineSeparator()));
//                    String confirmMsg = confirmRevisedCatMsg.substring(0 , confirmRevisedCatMsg.lastIndexOf("!"));
...
//                    String confirmMsg1 = confirmRevisedCatMsg.substring(confirmRevisedCatMsg.lastIndexOf("Bu") , confirmRevisedCatMsg.lastIndexOf(")"));
    ...
    @SuppressLint("RtlHardcoded")
    private void createRatingBar(final FormSectionInfo formSectionInfo, final FormFieldInfo fieldInfo, LinearLayout parentView, LinearLayout imageLayout, final HashMap<String, HashMap<String, Boolean>> sectionRequired, final PFAViewsCallbacks pfaViewsCallbacks2, LayoutInflater inflater) {

        @SuppressLint("InflateParams") ConstraintLayout rating_bar_ll = (ConstraintLayout) inflater.inflate(R.layout.rating_bar_ll, null, false);

        MaterialRatingBar ratingBar = rating_bar_ll.findViewById(R.id.ratingBar);
/*
        final PFAButton button = new PFAButton(mContext, fieldInfo, R.style.white_15_sp);
        button.setOnClickListener(new View.OnClickListener() {
            @Override
...
*/
    ...
    @SuppressLint("ClickableViewAccessibility")
    private void createMultiGoogleMap(LinearLayout parentView, LayoutInflater inflater, FormFieldInfo fieldInfo) {
        @SuppressLint("InflateParams") RelativeLayout map_detail_ll = (RelativeLayout) inflater.inflate(R.layout.map_detail_ll_large, null, false);
    ...
    private void createViewHeading(final LinearLayout parentView, LayoutInflater inflater, final FormFieldInfo fieldInfo) {
        ...
        Log.d("createViewGetCodeButton", "createViewHeading = " + fieldInfo.getField_name());
...
//            product_categoryDD.setRequired(true);
    ...
    private void createViewEditText(final LinearLayout parentView, final FormFieldInfo fieldInfo, PFATextInputLayout textInputLayout) {
        ...
        mContext.getSharedPreferences("fieldsPrefs", Context.MODE_PRIVATE).edit().putBoolean(fieldInfo.getField_name(), fieldInfo.isShowHideNotRequired()).apply();
...
//                    textInputLayout.setBackgroundColor(mContext.getResources().getColor(R.color.text_light_grey));
                    ...
                    Log.d("revisedLicesneUrlll", "text123 = " + revisedLicesneUrl);
                }
                if (fieldInfo.getField_name().equals("license_type_id")) {
                    pfaEditText.setClickable(false);
                    pfaEditText.setEnabled(false);
                    pfaEditText.setFocusable(false);
                    pfaEditText.setBackgroundColor(mContext.getResources().getColor(R.color.text_light_grey));
                ...
                if (fieldInfo.getField_name().equals("revised_license_type")) {
                    ...
                    txtCalulateCategory = new TextView(mContext);
                    txtCalulateCategory.setText("Calculate License Category");
                    txtCalulateCategory.setTextColor(mContext.getResources().getColor(R.color.white));
                    txtCalulateCategory.setBackgroundColor(mContext.getResources().getColor(R.color.colorPrimary));
                    txtCalulateCategory.setLayoutParams(params);
                    txtCalulateCategory.setTextSize(mContext.getResources().getDimension(R.dimen._5sdp));
                    txtCalulateCategory.setPadding(0, 30, 0, 30);
                    txtCalulateCategory.setGravity(Gravity.CENTER_HORIZONTAL);
                    txtCalulateCategory.setTypeface(null, Typeface.BOLD);

                    parentView.addView(txtCalulateCategory);

                    txtCalulateCategory.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
...
//                                                }
                                ...
                                if (wholesaler_investment != null && wholesaler_avg_sale != null && wholesaler_rent != null &&
                                    ...
                                    retailer_val.add(wholesaler_investment);
                                    retailer_val.add(wholesaler_avg_sale);
                                    retailer_val.add(wholesaler_rent);
                                    retailer_val.add(wholesaler_employees);
                                    retailer_val.add(wholesaler_location);
                                    retailer_val.add(wholesaler_bills);

                                    retailer_val_key.add(wholesaler_investment_key);
                                    retailer_val_key.add(wholesaler_avg_sale_key);
                                    retailer_val_key.add(wholesaler_rent_key);
                                ...
                                Log.d("liscentype", "Milk Producers");
                                if (mp_animals != null) {


                                    Log.d("liscentype", "Milk Producers fields not null");
                                    retailer_val.clear();
                                    retailer_val_key.clear();
                                    ...
                                    new PopulateLicenseCategory(mContext, revisedLicesneUrl, retailer_val, retailer_val_key, parent_license_type, parent_license_type_key, new CheckUserCallback() {
                                        ...
                                        @Override
                                        public void getExistingBusiness(JSONArray jsonArray, String msg) {
                                            Log.d("liscentype", "Milk Producers fields response");
                                            confirmRevisedCatMsg = msg;
                                            for (int i = 0; i < jsonArray.length(); i++) {
                                                ...
                                                if (parentView.findViewWithTag(jsonObject.optString("key")) instanceof PFAEditText) {
                                                    ...
                                                    Log.d("liscentype", "Milk Producers response not null");
                                                }
                                    ...
                                    sharedPrefUtils.showMsgDialog("Please fill above Milk Producers assessment dropdowns", null);
                            }
...
//        }
    ...
    private void createAutoSearchView(final LinearLayout parentView, LayoutInflater inflater, final FormFieldInfo fieldInfo) {
        ...
        autoSearchPFAET.setTextInputLayout(textInputLayout);
    ...
    private void createViewImageView(final LinearLayout parentView, LayoutInflater inflater, final FormSectionInfo formSectionInfo, final FormFieldInfo fieldInfo, LinearLayout imageLayout, final List<FormFieldInfo> fields, final HashMap<String, HashMap<String, Boolean>> sectionRequired, final PFAViewsCallbacks pfaViewsCallbacks2, int fieldCount) {
        ...
        if (fieldInfo.isAdd_more()) {
            ...
            addMoreImgBtn.setOnClickListener(v -> {
                ...
                createViews(formSectionInfo, parentView, sectionRequired, pfaViewsCallbacks2, true, mainScrollView , mContext);
                /*if (parentView.findViewWithTag("duplicate_clientid") instanceof PFASearchACTV){
                    PFASearchACTV pfaSearchACTV = parentView.findViewWithTag("duplicate_clientid");
                    PFADDACTV pfaddactv = parentView.findViewWithTag("status");
                    PFADDACTV pfaddactv1 = parentView.findViewWithTag("call_status");
                    if (pfaSearchACTV.textInputLayout.getVisibility() == VISIBLE || isCallStatusDUplicate || isStatusDUplicate){
                        pfaSearchACTV.formFieldInfo.setRequired(true);
                        pfaSearchACTV.showHideDropDown(true);
                    }
                    else {
                        pfaSearchACTV.formFieldInfo.setRequired(false);
                        pfaSearchACTV.showHideDropDown(false);
                    }
                }*/
...
//                }
    ...
    public void showHideCategoryFields(LinearLayout parentView, String catType) {
        switch (catType) {
            case "Retailer":
...
//                pfa_dd_actv.formFieldInfo.setInvisible(false);
            ...
            case "Rendering Units":
                ...
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        if (txtCalulateCategory != null)
                            txtCalulateCategory.setVisibility(GONE);

                        Log.d("liscentype", "Rendering Units dd onChange handler");
                        if (parentView.findViewWithTag("retailer_investment") instanceof PFADDACTV) {
                            Log.d("liscentype", "Rendering Units dd onChange retailer_investment");
                            PFADDACTV pfaddactv = parentView.findViewWithTag("retailer_investment");
                            pfaddactv.setVisibility(GONE);
                            pfaddactv.showHideDropDown(false);
                        }
                        ...
                        if (parentView.findViewWithTag("fat_rendering_license_type") instanceof PFADDACTV) {
                            PFADDACTV pfaddactv = parentView.findViewWithTag("fat_rendering_license_type");
                            if (pfaddactv != null) {
                                Log.d("liscentype", "fat_rendering_license_type not null 1");
                                pfaddactv.setVisibility(VISIBLE);
                                pfaddactv.showHideDropDown(true);
//                                    pfaddactv.showHideDropDownVisibility(false);
                            }
                        }
                        if (parentView.findViewWithTag("revised_license_type") instanceof PFAEditText) {
                            PFAEditText pfaddactv = parentView.findViewWithTag("revised_license_type");
    ...
    private void createViewDropdown(final FormFieldInfo fieldInfo, final LinearLayout parentView, LayoutInflater inflater) {
        ...
        if (fieldInfo.isMultiple()) {
            HashMap<String, HashMap<String, Boolean>> sectionRequired = new HashMap<>();
            PFAMultiSpinner pfaMultiSpinner = null;
            pfaMultiSpinner = new PFAMultiSpinner(mContext, fieldInfo, new MultiSpinnerListener() {
                @Override
                public void onItemsSelected(boolean[] selected, FormFieldInfo formFieldInfo, List<FormDataInfo> selectedValues) {
                    if (selectedValues.size() > 0) {

                        Log.d("checkDropDownMultii", "getRequired_false_fields = " + formFieldInfo.getRequired_false_fields());

                        List<String> fieldsToShow = new ArrayList<>();
                        List<String> fieldsToHide = new ArrayList<>();

                        for (int i = 0; i < selectedValues.size(); i++) {
                            mContext.getSharedPreferences("fieldsPrefs", Context.MODE_PRIVATE).edit().putString("milk_consumption" + i, selectedValues.get(i).getValue()).apply();
                            for (int j = 0; j < formFieldInfo.getShow_hidden_false_fields().size(); j++) {
                                if (selectedValues.get(i).getValue().equals(formFieldInfo.getShow_hidden_false_fields().get(j).getCheckKey())) {
                                    fieldsToShow.addAll(formFieldInfo.getShow_hidden_false_fields().get(j).getCheckViews());
                                }
                            }
                        }
                        mContext.getSharedPreferences("fieldsPrefs", Context.MODE_PRIVATE).edit().putInt("milk_consumption_size", selectedValues.size()).apply();
                        for (String value : formFieldInfo.getRequired_false_fields()) {
                            if (!fieldsToShow.contains(value)) {
                                fieldsToHide.add(value);
                            }
                        }

                        Log.d("checkDropDownMultii", "fieldsToShow = " + fieldsToShow);
                        Log.d("checkDropDownMultii", "fieldsToHide = " + fieldsToHide);

                        SharedPrefUtils sharedPrefUtils = new SharedPrefUtils(mContext);
                        if (formFieldInfo.getRequired_false_fields() != null) {
                            sharedPrefUtils.saveRLF((ArrayList<String>) formFieldInfo.getRequired_false_fields());
                        }

//                            Check fields to set them required and visible
                        if (formFieldInfo.getField_name() != null && (formFieldInfo.getShow_check_value() != null)
                                && (formFieldInfo.getShow_check_value().size() > 0)) {

                            if (selectedValues.size() > 0) {
                                formFieldsHideShow.setMultiFieldsRequiredAndVisible(fieldsToShow, true, sectionRequired, parentView, fieldsToHide);
                            }
                        }
                    }
                }
...
//                                parentView.removeView(sampleBrandNameDD);
                ...
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        if (parentView.findViewWithTag("duplicate_clientid") instanceof PFASearchACTV) {
                            PFASearchACTV pfaddactv1 = parentView.findViewWithTag("duplicate_clientid");
                            if (pfaddactv1 != null) {
                                pfaddactv1.setVisibility(GONE);
//                                pfaddactv1.formFieldInfo.setRequired(false);
                                pfaddactv1.showHideDropDown(false);
                                pfaddactv1.setText(" ");
                            }
                        }
                ...
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        if (parentView.findViewWithTag("parent_license_type") instanceof PFADDACTV) {
                            PFADDACTV pfaddactv56 = parentView.findViewWithTag("parent_license_type");
                            Log.d("duplicateEntry123", "value = " + pfaddactv56.formFieldInfo.getValue());
                            if (pfaddactv56.formFieldInfo.getDefault_value().equals("Rendering Units")) {

                                if (parentView.findViewWithTag("fat_rendering_license_type") instanceof PFADDACTV) {
                                    PFADDACTV pfaddactv = parentView.findViewWithTag("fat_rendering_license_type");
                                    if (pfaddactv != null) {
                                        Log.d("duplicateEntry123", "fat_rendering_license_type not null 1");
                                        pfaddactv.setVisibility(VISIBLE);
                                        pfaddactv.showHideDropDown(true);
                                    }
                                }
                                ...
                                if (parentView.findViewWithTag("fat_rendering_license_type") instanceof PFADDACTV) {
                                    PFADDACTV pfaddactv = parentView.findViewWithTag("fat_rendering_license_type");
                                    if (pfaddactv != null) {
                                        Log.d("duplicateEntry123", "fat_rendering_license_type not null 2");
                                        pfaddactv.setVisibility(GONE);
                                        pfaddactv.showHideDropDown(false);
                                    }
                                }
...
//                                    if (textView_name != null)
                            ...
                            if (id.equals("7")) {
                                ...
                                Log.d("duplicateEntry", "call_status 7");
                                if (parentView.findViewWithTag("duplicate_clientid") instanceof PFASearchACTV) {
                                    PFASearchACTV pfaddactv1 = parentView.findViewWithTag("duplicate_clientid");
//                                    FrameLayout frameLayout = (FrameLayout) pfaddactv1.getParent();
//                                    frameLayout.setVisibility(VISIBLE);
//                                    pfaddactv1.setVisibility(VISIBLE);
                                    if (pfaddactv1 != null) {
                                        Log.d("duplicateEntry", "call_status 7 not null");
//                                        pfaddactv1.formFieldInfo.setRequired(true);
                                        pfaddactv1.setVisibility(VISIBLE);
                                        pfaddactv1.showHideDropDown(true);
                                        pfaddactv1.setText("");
                                    }
//                                    pfaddactv1.setRequired(true);
                                ...
                                isCallStatusDUplicate = false;
                                Log.d("duplicateEntry", "call_status other");
                                PFASearchACTV pfaddactv1 = parentView.findViewWithTag("duplicate_clientid");
//                                FrameLayout frameLayout = (FrameLayout) pfaddactv1.getParent();
//                                frameLayout.setVisibility(GONE);
//                                pfaddactv1.setVisibility(GONE);
                                if (pfaddactv1 != null) {
                                    Log.d("duplicateEntry", "call_status other not null");
//                                    pfaddactv1.formFieldInfo.setRequired(false);
                                    pfaddactv1.setVisibility(GONE);
                                    pfaddactv1.showHideDropDown(false);
                                    pfaddactv1.setText(" ");
...
//                                    pfaddactv1.setRequired(true);
                                ...
                                Log.d("duplicateEntry", "status other");
                                PFASearchACTV pfaddactv1 = parentView.findViewWithTag("duplicate_clientid");
//                                FrameLayout frameLayout = (FrameLayout) pfaddactv1.getParent();
//                                frameLayout.setVisibility(GONE);
//                                pfaddactv1.setVisibility(GONE);
...
//                                pfaddactv1.setRequired(false);
                            ...
                            new PopulateParentLicense(mContext, parentBusinessCatUrl, id, new CheckUserCallback() {
                                ...
                                @Override
                                public void getExistingBusiness(JSONArray jsonArray, String msg) {
                                    Log.d("liscentype", "business category field repsone parent_business_category");
                                    ...
                                    } else if (parentView.findViewWithTag("revise_business_category") instanceof PFADDACTV) {
                                        PFADDACTV pfaddactv = parentView.findViewWithTag("revise_business_category");
                                        pfaddactv.setText("");
                                        pfaddactv.setSelectedValues(null);
...
//                                        Log.d("BusinessCattype", "array i = " + i);
                                    ...
                                    PFADDACTV pfaddactv = parentView.findViewWithTag("business_category");
                                    PFADDACTV pfaddactv1 = parentView.findViewWithTag("revise_business_category");
//                                    data.add(formDataInfo);
                                    if (parentView.findViewWithTag("business_category") instanceof PFADDACTV) {
                                        ...
                                        pfaddactv1.formFieldInfo.setData(data);
                                        pfaddactv1.listItemNames.clear();
                                        pfaddactv1.listItemNames = listItemNames;
                                        Log.d("BusinessCattype", "data size = " + data.size());
                                    }
                        ...
                        showHideCategoryFields(parentView, id);
                                ...
                                public void getExistingBusiness(JSONArray jsonArray, String msg) {
                                    ...
                                    for (int i = 0; i < jsonArray.length(); i++) {
                                        ...
                                        try {
                                            ...
                                            showHideCategoryFields(parentView, jsonObject.getString("value"));
                                        } catch (JSONException e) {
                                            e.printStackTrace();
                                        }
                                        ...
                                        if (parentView.findViewWithTag(jsonObject.optString("key")) instanceof PFADDACTV) {
                                            ...
                                            try {
                                                ...
                                                switch (jsonObject.getString("value")) {
                                                    case "Retailer":
                                                        pfaEditText.setDropdownSelection(0, pfaViewsCallbacks);
                                                        break;
                                                    case "Manufacturer":
                                                        pfaEditText.setDropdownSelection(1, pfaViewsCallbacks);
                                                        break;
                                                    case "Restaurants":
                                                        pfaEditText.setDropdownSelection(2, pfaViewsCallbacks);
                                                        break;
                                                    case "E-Commerce":
                                                        pfaEditText.setDropdownSelection(3, pfaViewsCallbacks);
                                                        break;
                                                    case "Rendering Units":
                                                        pfaEditText.setDropdownSelection(4, pfaViewsCallbacks);
                                                        /*PFAEditText pfaEditText1 = parentView.findViewWithTag("revised_license_type");
                                                        pfaEditText1.getFormFieldInfo().setInvisible(false);
                                                        pfaEditText1.getFormFieldInfo().setRequired(false);
                                                        pfaEditText1.setText("asd ");*/
                                                        Log.d("parentLicCat", "Rendering Units parent dd onChange 111");
                                                        break;
                                                    case "Wholesalers":
                                                ...
                                                if (!jsonObject.optBoolean("is_editable")) {
                                                    Log.d("liscentype", " revise_business_category not editable");
                                                    pfaEditText.setEnabled(false);
                                                    pfaEditText.setClickable(false);
//                                                        pfaEditText.setFocusable(false);
                                                    ...
                                                    Log.d("liscentype", " revise_business_category editable");
                                                    pfaEditText.setEnabled(true);
                                                    pfaEditText.setClickable(true);
//                                                        pfaEditText.setFocusable(true);
...
//                                                    pfaEditText.setBackgroundColor(mContext.getResources().getColor(R.color.white));
                        ...
                        if (fieldInfo.getField_name().equals("wholesaler_investment")) {
                            wholesaler_investment_key = "wholesaler_investment";
                            wholesaler_investment = id;
                        }
                        if (fieldInfo.getField_name().equals("wholesaler_avg_sale")) {
                            wholesaler_avg_sale_key = "wholesaler_avg_sale";
                            wholesaler_avg_sale = id;
                        }
                        if (fieldInfo.getField_name().equals("wholesaler_rent")) {
                            wholesaler_rent_key = "wholesaler_rent";
                            wholesaler_rent = id;
                        }
                        if (fieldInfo.getField_name().equals("wholesaler_employees")) {
                            wholesaler_employees_key = "wholesaler_employees";
                            wholesaler_employees = id;
                        }
                        if (fieldInfo.getField_name().equals("wholesaler_location")) {
                            wholesaler_location_key = "wholesaler_location";
                            wholesaler_location = id;
                        }
                ...
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (dropDownId != null && !dropDownId.isEmpty()) {
                            ...
                            showHideCategoryFields(parentView, dropDownId);
                        }
                    }
                }, 300);
            }
                        ...
                        if (pfaddactv1.formFieldInfo != null && pfaddactv1.formFieldInfo.getField_name() != null && (pfaddactv1.formFieldInfo.getShow_check_value() != null)
                            ...
                            if (pfaddactv1.getSelectedValues() != null && pfaddactv1.getSelectedValues().size() > 0) {
                                ...
                                if (pfaddactv1.formFieldInfo.getShow_check_value().contains(pfaddactv1.getSelectedValues().get(0).getValue())) {
                                    ...
                                    Log.d("parentLicCat", "Rendering Units parent dd onChange here1 if check = " + pfaddactv1.getSelectedValues().get(0).getValue());
                                    Log.d("parentLicCat", "Rendering Units parent dd onChange here1 if get check = " + pfaddactv1.formFieldInfo.getShow_check_value());
    ...
    private void setFoodLabViews(final LinearLayout menuFragParentLL, List<
        ...
        if (menuFragParentLL.findViewWithTag(DD_FOOD_LAB_TEST + "LL") != null) {
            ...
            for (FormSectionInfo formSectionInfo : formSectionInfos) {
                createViews(formSectionInfo, ddFoodLabTestLL, sectionRequired, null, false, fragMenuItemSV , mContext);
            }
        }
